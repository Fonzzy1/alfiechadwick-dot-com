...
![SIR Diagram](/post/1-Drug-Use_Models/pics/1.png)
...
![SUAR Diagram](/post/1-Drug-Use_Models/pics/2.png)
...
![SUAR Diagram with ageing](/post/1-Drug-Use_Models/pics/3.png)
...
for i, a in enumerate(p):
    for j, b in enumerate(p):
        if i != j:
            ...
            t = axs[i, j].pcolormesh(w, cmap="RdYlGn_r", vmin=0, vmax=216)  # Here is the change
        if i == 4:
...
for i , a in enumerate( p):
    for j , b in enumerate(p):
        if i !=j:
            ...
            t = axs[i, j].pcolormesh(w, cmap="RdYlGn_r", vmin=0, vmax=1)  # Here is the change
        if i == 4:
            axs[i,j].set_xticks(np.arange(7) + 0.5, minor=False)
            axs[i,j].set_xticklabels(xlabels, fontdict=None, minor=False)
            axs[i,j].set_xlabel('Log2(' + b +')')
